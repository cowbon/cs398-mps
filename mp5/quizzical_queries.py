...
def query_1(sc, sqlContext):
    ...
    '''
    return sqlContext.sql('SELECT Text FROM table WHERE Id=22010').collect()[0]['Text']
...
def query_2(sc, sqlContext):
    ...
    '''
    return sqlContext.sql('SELECT count(*) FROM table WHERE ProductId="B000E5C1YE" AND Score=5').collect()[0]['count(1)']
...
def query_3(sc, sqlContext):
    ...
    '''
    return sqlContext.sql('SELECT count(distinct UserId) FROM table ').collect()[0]['count(DISTINCT UserId)']
